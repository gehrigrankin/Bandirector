...
import { Play, Pause, ArrowLeft, Music4 } from "lucide-react";
...
export function SongCoach({ song }: { song: Song }) {
  ...
  const analysisMessage =
    song.status === "failed"
      ? "Analysis failed for this song, so we can't break it into practice sections yet."
      : song.status === "ready"
        ? "No chords were found for this song yet."
        : "Still analyzing this song — chords and sections will show up here once it's done.";
  ...
  const emptyState = (
    <div className="flex flex-1 flex-col items-center justify-center gap-3 px-6 py-10 text-center">
      <div className="flex size-12 items-center justify-center rounded-full bg-bg-higher text-text-dim">
        <Music4 className="size-5" strokeWidth={1.8} />
      </div>
      <div>
        <div className="font-display text-lg font-semibold">No lesson yet</div>
        <p className="mt-1.5 max-w-xs text-[13px] leading-relaxed text-text-muted">
          {analysisMessage}
        </p>
      </div>
      <Link
        href={`/songs/${song.id}/edit`}
        className="mt-1 inline-flex h-9 items-center justify-center rounded-xl border border-line px-4 text-xs font-semibold text-text-soft hover:bg-bg-higher"
      >
        Edit chords manually
      </Link>
    </div>
  );
        ...
        {!hasSections ? (
          ...
          <>
            ...
            <div className="flex min-h-0 flex-1 flex-col items-center justify-center gap-3.5 px-5">
              ...
              <div className="font-display text-[clamp(72px,24vw,96px)] font-semibold leading-none text-jam">
