...
export default async function LibraryPage() {
  ...
  return <AppShell initials={initials}><main className="mx-auto min-h-full max-w-6xl px-5 py-7 md:px-12 md:py-10"><header><p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-text-dim">Your music</p><h1 className="mt-2 font-display text-3xl font-bold">Songs</h1><p className="mt-2 max-w-xl text-sm leading-relaxed text-text-muted">Every song is a workspace: practice it, fix it, learn from it, or bring the band in.</p></header><div className="mt-7"><SongsLibrary songs={rows} favoriteIds={(favs ?? []).map((fav) => fav.song_id)} /></div></main></AppShell>;
}
